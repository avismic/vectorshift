...
def run_pipeline(pipeline: str = Form(...)):
    ...
    while queue:
        ...
        if node_type in node_executors:
            ...
            # --- SPECIAL LOGIC FOR CONDITIONAL ROUTING ---
            if node_type == 'conditional':
                condition_met = result.get('condition_met', False)
                passthrough_value = result.get('passthrough_value')
                # Determine the correct output handle based on the condition result
                ...
                for neighbor_id in adj[node_id]:
                    ...
                    source_handle = edge.get('sourceHandle', '')
                    # Check if the edge's source handle matches the path we want to follow
                    if handle_to_follow in source_handle:
                        print(f"    > Routing to {neighbor_id}")
                        neighbor_node = nodes[neighbor_id]
                        if 'data' not in neighbor_node: neighbor_node['data'] = {}
                        # Pass the original, unmodified value down the correct path
            ...
            # --- STANDARD LOGIC FOR ALL OTHER NODES ---
                ...
                for neighbor_id in adj[node_id]:
                    ...
                    if source_handle:
                        parts = source_handle.split('-')
                        if len(parts) > 2:
                            key = parts[-3] # Use -3 to get the correct key
                            if isinstance(result, dict) and key in result:
                                value_to_pass = result[key]
                    ...
                    if 'data' not in neighbor_node: neighbor_node['data'] = {}
                    # This is the crucial part for passing data to the next node
                    if 'value' in neighbor_node['data']:
                        # If the neighbor already has a value, we don't overwrite it
                        # unless we have a specific value to pass. This is a placeholder
                        # for more complex merging logic in the future.
